...
class MLEPolicy(nn.Module):
    def __init__(self,s_dim,cfg,a_dim,):
        super(MLEPolicy,self).__init__()
        h_dim = cfg['h_dim']
        self.policy = MultiDiscretePolicy(s_dim,h_dim,a_dim)

        self.clf_loss_fct = torch.nn.BCEWithLogitsLoss(pos_weight=[h_dim,a_dim])

    def forward(self,s):
        a_weights = self.policy.load_state_dict(torch.load(self.model_config['pretrained_weights']))
        return a_weights

    def select_action(self,s,ample=True):
        return self.policy.select_action(s,sample=True)

    def get_log_prob(self,s,a):
        return self.policy.get_log_prob(s,a)
